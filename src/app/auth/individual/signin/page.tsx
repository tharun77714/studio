...
function ShimmerButton({
  ...
  return (
    <motion.button
      ...
      className="relative w-full h-12 rounded-xl font-sans font-semibold text-sm overflow-hidden cursor-pointer select-none disabled:opacity-50 disabled:cursor-not-allowed"
      style={{
        background: `linear-gradient(135deg, rgba(${ACCENT_RGB},0.9), rgba(${ACCENT_RGB},0.7))`,
        color: "#ffffff",
        boxShadow: `0 4px 20px rgba(${ACCENT_RGB},0.3), 0 1px 0 rgba(255,255,255,0.1) inset`,
      }}
    >
      <motion.div
    ...
    <div className="flex items-center gap-2">
      <div className="relative w-8 h-8">
        <svg className="w-8 h-8 -rotate-90" viewBox="0 0 32 32">
          ...
          <motion.circle cx="16" cy="16" r="12" fill="none" stroke={ACCENT} strokeWidth="2.5" strokeLinecap="round"
            strokeDasharray={75.4} strokeDashoffset={75.4 * (1 - pct / 100)} transition={{ duration: 0.5 }} />
        </svg>
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="w-1.5 h-1.5 rounded-full" style={{ background: ACCENT }} />
        </div>
      </div>
      <span className="text-xs font-sans tabular-nums" style={{ color: ACCENT }}>
        {mins}:{secs < 10 ? "0" : ""}{secs}
      </span>
    </div>
  );
...
function IndividualSignInContent() {
  ...
  async function onEmailSubmit(data: EmailForm) {
    ...
    try {
      ...
      toast({ title: "Welcome to Sparkle Studio!", description: "Accessing your dashboard..." });
  ...
  async function onOTPSubmit() {
    ...
    try {
      const res = await fetch("/api/auth/phone/verify", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ phone: phoneNumber, code: otpValue, expectedRole: "individual" }),
      });
      const result = await res.json();
      if (!res.ok) throw new Error(result.error);
      await refreshProfile(result);
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      const params = new URLSearchParams({
        ...
        access_type: "offline", prompt: "select_account", state: "individual",
      });
    ...
    <div className="min-h-screen w-full flex" style={{ background: "#030303" }}>
      ...
      <div className="flex-1 flex items-center justify-center p-8 lg:p-12 relative">
        <div
          className="absolute inset-0 pointer-events-none"
          style={{
            background: "linear-gradient(135deg, rgba(139,92,246,0.02) 0%, transparent 60%)",
        ...
        >
          ...
          <motion.div variants={staggerContainer} initial="initial" animate="animate" className="mb-8">
            <motion.div variants={fadeUp} className="flex items-center gap-2 mb-6">
              <Link href="/" className="text-neutral-500 hover:text-neutral-300 text-xs font-sans transition-colors">
                Sparkle Studio
              </Link>
              <ChevronRight className="w-3 h-3 text-neutral-600" />
              <span className="text-neutral-400 text-xs font-sans">Sign In</span>
            </motion.div>
            ...
            <motion.h2 variants={fadeUp} className="font-headline text-3xl font-bold text-white mb-2 tracking-tight">
              ...
              <span
                className="block"
                style={{
                  background: `linear-gradient(135deg, ${ACCENT}, rgba(167,139,250,0.7))`,
            ...
            <motion.p variants={fadeUp} className="text-neutral-500 font-sans text-sm">
              Sign in to your personal Sparkle Studio account
            </motion.p>
          </motion.div>
            ...
            >
              {[
                ...
                return (
                  <button
                    key={tab.id}
                    onClick={() => switchTab(tab.id as "email" | "phone")}
                    className="relative flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-lg text-sm font-sans font-medium z-10 transition-colors duration-200"
                    style={{ color: isActive ? "#ffffff" : "rgba(156,163,175,0.9)" }}
                  >
                    {isActive && (
                      <motion.div
                        layoutId="individual-tab-bg"
                        className="absolute inset-0 rounded-lg"
                        style={{ background: `rgba(${ACCENT_RGB},0.8)` }}
          ...
          <AnimatePresence mode="wait" custom={tabDir}>
            {activeTab === "email" ? (
              <motion.div key="email" custom={tabDir} variants={tabContent} initial="initial" animate="animate" exit="exit">
                <form onSubmit={emailForm.handleSubmit(onEmailSubmit)} className="space-y-4">
                  ...
                  <AnimatedInput
                    ...
                    rightElement={
                      <button type="button" onClick={() => setShowPassword(!showPassword)} className="text-neutral-400 hover:text-white transition-colors">
                  ...
                  <div className="flex justify-end">
                    <Link href="/auth/forgot-password" className="text-xs font-sans transition-colors" style={{ color: `rgba(${ACCENT_RGB},0.8)` }}>
                      Forgot password?
                    </Link>
                  </div>
              ...
              <motion.div key="phone" custom={tabDir} variants={tabContent} initial="initial" animate="animate" exit="exit">
                <AnimatePresence mode="wait">
                  {phoneState === "idle" ? (
                    <motion.div key="send" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4">
                      <div className="space-y-2">
                        <label className="text-xs font-sans text-neutral-400 tracking-wide uppercase pl-1">Mobile Number</label>
                      ...
                      <ShimmerButton onClick={onPhoneSend} disabled={isLoading}>
                    ...
                    <motion.div key="verify" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-5">
                      ...
                      <ShimmerButton onClick={onOTPSubmit} disabled={isLoading || otpValue.length < 6}>
          ...
          <div className="relative flex items-center my-5">
            ...
            <span className="mx-4 text-[11px] font-sans text-neutral-500 tracking-widest uppercase">or</span>
            <div className="flex-1 h-px bg-white/[0.07]" />
          </div>
          <GhostButton onClick={handleGoogleSignIn} disabled={isLoading}>
          ...
          <p className="text-center text-sm text-neutral-500 font-sans mt-6">
            ...
            <Link href="/auth/individual/signup" className="font-semibold transition-colors hover:opacity-80" style={{ color: ACCENT }}>
              Create account
            </Link>
          </p>
        </motion.div>

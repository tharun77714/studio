...
import { ArrowLeft, Gem, Sparkles, Mail, Phone, Chrome } from "lucide-react";
...
export default function BusinessAuthPage() {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await handleSuccess({ id: "business_123" });
    } catch(e:any) {
    ...
    <div className="mb-8 z-20 relative">
      ...
      <div className="flex items-center gap-2.5 mb-6">
        <div className="w-10 h-10 rounded-2xl flex items-center justify-center bg-white shadow-[0_2px_10px_rgba(0,0,0,0.06)] border border-black/[0.05]">
          <Gem className="w-5 h-5 text-[hsl(262,83%,58%)]" />
        </div>
        <div>
  ...
  const FrontContent = (
    <>
      <Header title={frontMode === "phone" ? "Phone Auth" : "Welcome Back"} subtitle="Secure access to your business workspace." />
      ...
      <div className="flex-1 relative">
        <AnimatePresence mode="wait">
          {frontMode === "email" ? (
            ...
            >
              <LuxuryInput label="Work Email" type="email" required />
  ...
  const BackContent = (
    <>
      <Header title="Create Account" subtitle="Join Sparkle Studio Business." />
      <div className="flex-1 relative flex flex-col justify-center">
        ...
        >
          ...
          <LuxuryInput label="Company Name" required />
          <LuxuryInput label="Work Email" type="email" required />
          <LuxuryInput label="Password" type="password" required />
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 overflow-hidden bg-[#050505]">
      <AuroraBackground variant="violet" isSuccess={isSuccess} />

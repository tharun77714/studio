...
import {
  ...
  Phone, ShieldCheck, MessageSquare, LogIn, Gem,
} from "lucide-react";
...
function PrimaryBtn({
  ...
  return (
    <motion.button ref={ref} type={type} disabled={disabled} onClick={fire}
      ...
      className="relative w-full h-12 rounded-2xl font-sans font-semibold text-[13px] overflow-hidden select-none disabled:opacity-50 disabled:cursor-not-allowed"
      style={{ background: `linear-gradient(135deg,rgba(${ACCENT_RGB},0.95),rgba(${ACCENT_RGB},0.65))`, color: "#0a0700", boxShadow: `0 6px 28px rgba(${ACCENT_RGB},0.35),0 1px 0 rgba(255,255,255,0.18) inset` }}
    >
      <motion.div className="absolute inset-0 pointer-events-none"
        ...
        initial={{ backgroundPosition: "200% 0" }} whileHover={{ backgroundPosition: ["-200% 0","200% 0"] }}
        transition={{ duration: 0.6, ease: "easeInOut" }} />
      {ripples.map((r) => (
        <motion.span key={r.id} className="absolute rounded-full pointer-events-none"
          style={{ left: r.x, top: r.y, width: 8, height: 8, marginLeft: -4, marginTop: -4, background: "rgba(255,255,255,0.45)" }}
          initial={{ scale: 0, opacity: 0.9 }} animate={{ scale: 22, opacity: 0 }}
...
function GoogleBtn({ onClick, disabled }: { onClick?: () => void; disabled?: boolean }) {
  return (
    <motion.button type="button" disabled={disabled} onClick={onClick}
      ...
      className="relative w-full h-11 rounded-2xl font-sans font-medium text-[13px] text-white/80 hover:text-white flex items-center justify-center gap-3 transition-colors duration-200 overflow-hidden"
      style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)" }}
    >
      <motion.div className="absolute inset-0" initial={{ opacity: 0 }} whileHover={{ opacity: 1 }}
        style={{ background: `rgba(${ACCENT_RGB},0.05)` }} transition={{ duration: 0.2 }} />
      ...
      <span className="relative z-10">Continue with Google Workspace</span>
    </motion.button>
  );
}
  ...
  return (
    <div className="flex items-center gap-2">
      <svg className="w-7 h-7 -rotate-90" viewBox="0 0 28 28">
        ...
        <motion.circle cx="14" cy="14" r="10" fill="none" stroke={ACCENT} strokeWidth="2.5"
          strokeLinecap="round" strokeDasharray={62.8} strokeDashoffset={62.8*(1-pct)}
...
function BusinessSignInContent() {
  ...
  useEffect(() => {
    if (phoneState !== "sent" || otpTimer <= 0) {
      if (otpTimer === 0) { setPhoneState("idle"); toast({ title: "OTP Expired", description: "Request a new code.", variant: "destructive" }); }
      return;
    }
  ...
  async function onOTPSubmit() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(result.error);
      await refreshProfile(result);
      toast({ title: "Verified! Welcome back." });
      router.push("/dashboard");
        ...
        >
          ...
          <div className="px-8 pt-9 pb-8">
            ...
            <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.15, duration:0.4 }} className="mb-7">
              ...
              <p className="text-neutral-500 text-sm font-sans">Access your Sparkle Studio dashboard</p>
            </motion.div>
              ...
              {(["email","phone"] as const).map((tab, i) => {
                ...
                return (
                  <button key={tab} onClick={() => switchTab(tab)}
                    ...
                    style={{ color: active ? "#0a0700" : "rgba(156,163,175,0.85)" }}>
                    {active && (
                      <motion.div layoutId="biz-tab" className="absolute inset-0 rounded-lg"
            ...
            <div style={{ perspective: "900px" }}>
              <AnimatePresence mode="wait" custom={tabDir}>
                {activeTab === "email" ? (
                  <motion.div key="email" custom={tabDir}
                    variants={{ initial: slide(tabDir).initial, animate: slide(1).animate, exit: slide(tabDir).exit }}
                    initial="initial" animate="animate" exit="exit"
                    style={{ transformStyle:"preserve-3d" }}
                  >
                    <form onSubmit={emailForm.handleSubmit(onEmailSubmit)} className="space-y-4">
                      <AnimatedInput label="Business Email" type="email" icon={<Mail className="w-4 h-4"/>}
                        accentColor={ACCENT} accentRgb={ACCENT_RGB} placeholder="contact@company.com"
                      ...
                      <div className="flex justify-end -mt-1">
                        <Link href="/auth/forgot-password" className="text-xs font-sans hover:opacity-80 transition-opacity" style={{ color:`rgba(${ACCENT_RGB},0.75)` }}>
                          Forgot password?
                        </Link>
                      </div>
                  ...
                  <motion.div key="phone" custom={tabDir}
                    variants={{ initial: slide(tabDir).initial, animate: slide(1).animate, exit: slide(tabDir).exit }}
                    initial="initial" animate="animate" exit="exit"
                    style={{ transformStyle:"preserve-3d" }}
                  >
                    <AnimatePresence mode="wait">
                      {phoneState === "idle" ? (
                        <motion.div key="send" initial={{ x:30, opacity:0, filter:"blur(4px)" }} animate={{ x:0, opacity:1, filter:"blur(0px)" }} exit={{ x:-30, opacity:0, filter:"blur(4px)" }} transition={{ duration:0.3, ease:[0.25,0.1,0.25,1] }} className="space-y-4">
                        ...
                        <motion.div key="verify" initial={{ x:30, opacity:0, filter:"blur(4px)" }} animate={{ x:0, opacity:1, filter:"blur(0px)" }} exit={{ x:-30, opacity:0, filter:"blur(4px)" }} transition={{ duration:0.3, ease:[0.25,0.1,0.25,1] }} className="space-y-5">
            ...
            <p className="text-center text-sm text-neutral-500 font-sans mt-6">
              No account?{" "}
              <Link href="/auth/business/signup" className="font-semibold hover:opacity-80 transition-opacity" style={{ color:ACCENT }}>
                Register here
              </Link>
            </p>
          </div>
